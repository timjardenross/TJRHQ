...
1. No Amber ("orange") behaviour at all -- the function only branched on
   `!= "red"`, so Amber got identical (fully unrestricted) treatment to
   Green.
2. Unknown capacity (no check-in today, capacity_state is None) was
   silently treated as unrestricted/Green -- the exact "absence must not
   imply Green" risk Mission 2's brief explicitly warns against.
...
class TestAmberTrimsOnlyGentle:
    def test_amber_drops_far_due_gentle_item(self):
        kept = fte._apply_capacity_gate(ALL_MODES_FAR_DUE, "orange", NOW)
    ...
    def test_amber_keeps_near_due_gentle_item(self):
        ...
        kept = fte._apply_capacity_gate(near, "orange", NOW)
        assert len(kept) == 1
    ...
    def test_amber_keeps_gentle_item_with_no_due_date_if_not_urgent_by_other_signal(self):
        ...
        kept = fte._apply_capacity_gate(no_due, "orange", NOW)
        assert kept == []

    def test_amber_is_strictly_looser_than_red(self):
        red_kept = {t["id"] for t in fte._apply_capacity_gate(ALL_MODES_FAR_DUE, "red", NOW)}
        amber_kept = {t["id"] for t in fte._apply_capacity_gate(ALL_MODES_FAR_DUE, "orange", NOW)}
...
class TestUnknownCapacityIsNotTreatedAsGreen:
    """The core safety fix: capacity_state=None (no check-in today) must
    not silently behave like Green (fully unrestricted)."""

    def test_none_capacity_drops_far_due_gentle_item(self):
        kept = fte._apply_capacity_gate(ALL_MODES_FAR_DUE, None, NOW)
        assert "t-gentle" not in {t["id"] for t in kept}

    def test_none_capacity_behaves_identically_to_amber(self):
        none_kept = {t["id"] for t in fte._apply_capacity_gate(ALL_MODES_FAR_DUE, None, NOW)}
        amber_kept = {t["id"] for t in fte._apply_capacity_gate(ALL_MODES_FAR_DUE, "orange", NOW)}
        assert none_kept == amber_kept

    def test_none_capacity_is_not_identical_to_green(self):
        none_kept = {t["id"] for t in fte._apply_capacity_gate(ALL_MODES_FAR_DUE, None, NOW)}
        green_kept = {t["id"] for t in fte._apply_capacity_gate(ALL_MODES_FAR_DUE, "green", NOW)}
        assert none_kept != green_kept
