...
| Field | Value |
|---|---|
| **Registry Version** | 2.14 |
| **Last Architecture Review** | 2026-09-12 (USS-TJR-MSN-0365, platform hygiene sprint — added Observability as a new capability (Phoenix LLM tracing live since PR #109, `core/quality/garak_gate.py` pre-activation gate + `_ModelRouterJudge`-backed `score_output()` merged this pass); corrected Confidence's `quality_scores` status from "0 rows since test cleanup" to "table dropped outright in migration 0183 (2026-09-01), `score_outcome()`'s INSERT has been silently failing since" (found during PR #114); noted Scheduling's infra-timer side got a real bug fix (`auto_remediation.py`'s `git_commit()` — LL-149) though the apscheduler fragmentation itself is untouched. No other capability's figures touched by this pass.) |
| **Previous Review** | 2026-09-05 (added Voice Synthesis (Chatterbox TTS) as a new capability — built and deployed this session per Captain's request, systemd-live with one HTTP smoke test, zero adopters. No other capability's figures touched by this pass.) |
| **Platform Capability Count** | 34 (was 33; +1 for Observability) |
...
| **Capabilities by Maturity** | L1: 6 · L2: 20 · L3: 7 · L4: 1 · L5: 0 |
...
|---|---|---|---|---|---|---|---|---|---|---|
| Governance | Fragmented, consolidation continuing | L2 | 65% | Chief Engineer / Number One | Active (fragmented) | Medium | Y·Y·~·Y | Architectural Debt | Fix Later | ADR-registry consolidation: canonical dir now holds 10 filed ADRs (ADR-003/004/006/013/020/022/024/027/030/031, 4 explicitly low-confidence reconstructions). USS-TJR-MSN-0370 verified all ~23 remaining bare citations individually: only 1 (ADR-006) was real, 22 confirmed noise (fixture/test data, the stale UI mock, template boilerplate) — the ADR-number citation backlog is now fully resolved, not just partially covered |
...
| Observability | Phoenix LLM tracing live (all 7 call sites, since PR #109); garak pre-activation gate + judge-model-backed `score_output()` merged (USS-TJR-MSN-0365) | L2 | 60% | Chief Engineer | Active (Phoenix) / Degraded (garak gate — verified wiring only, real router unreachable from build sandbox) | Low | Y·Y·Y·~ | Operational Issue (garak gate not yet run against the live Model Router) | Fix Later | Run `core/quality/garak_gate.py` against the live `:8891` router on the VM; wire `score_output()` beyond its current shadow-mode-only caller |
...
| Confidence | Activated, narrow scope, verified end-to-end; `quality_scores` write path confirmed broken, not just quiet | L3 | 70% | Chief Engineer | Degraded | Medium (`score_outcome()`'s INSERT has been silently failing since 2026-09-01, no alerting) | Y·Y·Y·N (`quality_scores` table dropped in migration 0183 (2026-09-01) — found during PR #114; `score_outcome()` still calls `INSERT INTO quality_scores` every time, caught by its own broad `except`, never surfaced) | Operational Issue | Fix Now — either restore a write target for `score_outcome()` or retire the dead INSERT | Extend beyond research/comms into remaining decision domains once the write path is real again |
...
| Scheduling | Fragmented (5 apscheduler instances); infra-timer side (`hq-evolution.timer`) had a real automation bug, now fixed | L2 | 65% | TBD | Active (per-instance) | Medium (real double-fire risk, disclosed) | Y·Y·Y·Y | Architectural Debt | Fix Later | Wave 4 consolidation (mission not yet assigned); apscheduler fragmentation itself untouched by the LL-149 fix |
...
| Data Classification & Model Routing | USS-TJR-MSN-0366 Stream 5: real automatic PII/identifier detection + LLM01/LLM02 blocking built and code-committed on the Model Router's Gemini branch + 2 of 3 mistralai clients — not yet provisioned/observed live on the production host | L2 | 50% | Chief Engineer | Dormant (built, not yet deployed/provisioned on the production host) | Low (fails closed, not silent) | Y·~·~·N | Operational Gap | Fix Later | Provision `platform-runtime/.venv-llmsec` on the production host and verify the semantic rail against real Ollama; then extend to `mistral_agent_client.py` and evaluate a `:cloud`-aware wrap of `_ollama_generate()` |
...
| CI/CD & Supply-Chain Hygiene | USS-TJR-MSN-0369+0370: both bandit's Medium backlog and ruff's full backlog (0 findings repo-wide each) are triaged and both CI steps flipped to blocking. Real bugs caught in the ruff pass, not just lint noise (undefined-name NameError, blocking I/O in async handlers, dropped stats never reaching Captain briefs, a mislabeled-timezone staleness calc). Dependabot: all 11 PRs triaged; #157 (supabase) redesigned off the removed-internal monkeypatch onto a real public API, safe intermediate bump (2.7.4) shipped, full-target upgrade understood-blocked on a PTB dependency conflict. | L3 | 85% | Chief Engineer | Active (bandit + ruff gates both blocking) | Low | Y·Y·Y·Y | Healthy | Leave As-Is | `tg-xo.service` needs a real redeploy to pick up the supabase/gotrue fix (repo fix landed, host never reinstalled); `telegram-bots/revs`'s identical supabase monkeypatch is an easier follow-up (no httpx/PTB conflict); lcars-portal deps (#160/#162/#168) remain held on Vercel's deploy-rate-limit, deliberately not attempted this pass |
...
- **Technical Debt:** the "4 ADR registries" this line originally referred to (`governance/ADR-*.md`, `knowledge/Architectural-Decisions.md`, `architecture/decisions/`) no longer exist in the tree as of USS-TJR-MSN-0368's check (2026-09-12) — already cleaned up or never materialized as literal paths, unclear which. What's real instead: 18 distinct `ADR-NNN` numbers are cited by name across the codebase with zero backing files anywhere (the canonical target directory has been scanned by `platform-runtime/adr_conflict_detector.py` since it was written without ever finding anything). MSN-0368 formalized 2 of the 18 (ADR-020, ADR-027 — the two with real, repeated, unambiguous grounding); the other 16 are an inventory task, not yet started — see that mission's knowledge record for the full list and why the rest weren't fabricated from memory. D-prefix collision: checked, not found live — `governance/directives/` and `governance/decisions/` don't exist either; `id_registry.py`'s single `DEC` counter is the only live D-prefix minting path today.
- **Next Planned Evolution:** formalize the remaining ~16 referenced ADR numbers into `core/governance/architecture-decision-records/` (MADR format, template at `docs/decisions/TEMPLATE-madr.md` per USS-TJR-MSN-0366 Stream 10) as their real historical context can be reconstructed — do not invent content for one that can't be grounded in an existing decision.
...
- **Description:** seeing what the platform's LLM calls and quality/security posture actually are — trace spans for model calls, and gates that check output/system quality before or after the fact.
- **Purpose:** replace silent failure (a broken import that fails every trace for two weeks with no log line, a scoring function with zero callers) with something that surfaces state changes and lets a human or gate catch problems before they reach production traffic.
- **Engineering Confidence:** 60% — Phoenix tracing is genuinely live with a real span verified in the UI; the garak gate's wiring is proven correct (fails closed on a real connection error) but has never actually run against the live Model Router, so its true pass/fail verdict on this platform's own traffic is still unknown.
- **Current Maturity:** L2 — Implemented, first real gate merged, not yet exercised end-to-end in production.
- **Current Status:** Arize Phoenix (`phoenix.service`, `:6006` UI / `:4317` OTLP) live since PR #109 — fixed a `platform_runtime`/`platform-runtime` import-path break that had silently failed all 7 `configure_tracing()` call sites since 2026-08-29. `core/quality/garak_gate.py` (USS-TJR-MSN-0365 Stream C) added as a pre-activation LLM-vulnerability gate against the local Model Router (`:8891`), zero-tolerance threshold, reports to `reports/garak/`; verified in-sandbox that its `RestGenerator` wiring targets the router's real REST contract and fails closed when unreachable, but never run against the actual live router. `quality_scoring_service.py`'s `score_output()` (deepeval `HallucinationMetric`) got a real judge model (`_ModelRouterJudge`, backed by `core/model-router`'s `/api/model/escalate`) in place of deepeval's unconfigured OpenAI default, and its first real caller (shadow-mode only, default OFF) in `build_learning_loop.py`.
- **Owner:** Chief Engineer.
- **Canonical Implementation:** `tools/start-phoenix.sh` / `deploy/phoenix.service` (tracing); `core/quality/garak_gate.py` (vulnerability gate); `platform-runtime/lib/quality_scoring_service.py`'s `score_output()` + `_ModelRouterJudge` (per-output quality scoring).
- **Consumers:** all 7 `configure_tracing()` call sites (Phoenix); `build_learning_loop.py` (shadow-mode `score_output()` caller); none yet for the garak gate (not wired into any deploy/activation flow — it exists as a standalone script, not a required check).
- **Dependencies:** Arize Phoenix (`arize-phoenix`, OpenTelemetry OTLP exporters), `garak==0.17.0`, `deepeval>=4.1.10`, `core/model-router` (`:8891`, both the garak gate's target and the judge model's backend).
- **Capability Relationships:**
  - *Depends On:* Model Router (both the garak gate's scan target and `_ModelRouterJudge`'s backend).
  - *Consumes:* nothing shared yet.
  - *Produces:* trace spans (Phoenix), garak vulnerability reports (`reports/garak/`), per-output hallucination-rate quality scores (currently shadow-mode/observational only).
  - *Future Dependencies:* Confidence (a real, non-dropped write target for LLM-output quality scores would let `score_output()` graduate past shadow mode); Task Engine or a deploy pipeline (to make the garak gate an actual blocking pre-activation check rather than a standalone script).
- **Related Missions:** USS-TJR-MSN-0365 (platform hygiene sprint — garak gate, judge model, Phoenix systemd unit); PR #109 (Phoenix import-path fix), PR #114 (judge model + first caller), PR #170 (garak gate).
- **Technical Debt:** the garak gate is not wired into any activation/deploy flow — running it is currently a manual, opt-in step, not an enforced gate; `score_output()` has exactly one caller, in shadow mode, and nothing yet consumes its output beyond a log line; Phoenix's `call_log.jsonl` rotation remains a separate, still-open gap (not conflated with this capability's tracing fix).
- **Next Planned Evolution:** run `garak_gate.py` against the live Model Router on the actual VM to get a first real pass/fail verdict; decide whether the gate should block deploy/activation or stay advisory; graduate `score_output()` beyond shadow mode once Confidence's write-target question (see that capability's Technical Debt) is resolved.
...
- **Description:** local, self-hosted text-to-speech service using Resemble AI's open-source Chatterbox model, with voice-cloning support via a reference audio clip. **Re-scoped 2026-09-12 (USS-TJR-MSN-0366 Stream 7) to voice-cloning-only** — plain narration moved to Kokoro TTS (see next entry).
- **Purpose:** give workbenches and LifeOS a local, cost-free, voice-cloneable speech option (`voice_ref`) alongside the existing cloud `tts_edge.py` (Microsoft edge-tts) path — no per-call API cost, no data leaving the VM. No longer positioned for plain narration — 0.08x realtime CPU speed made that the wrong tool for anything with a real alternative.
- **Engineering Confidence:** 40% — one successful end-to-end generation via the live HTTP service confirms the mechanism works, but that's a single short-text smoke test: no voice-cloning tested, no concurrent-request behaviour tested, no adopter has exercised the cloning path itself.
- **Current Maturity:** L1 — Implemented, its one real caller (narration, `intelligence/scheduler.py`) moved off it to Kokoro; cloning path (its remaining scope) still has zero adopters.
- **Current Status:** systemd-managed, running. This VM is CPU-only (no GPU) — measured real-world speed is 0.08x realtime (a 4-second clip took 48 seconds to generate), far below upstream's advertised "3x realtime on 8 CPU cores" claim for this same Nano model. Confirmed too slow for narration specifically by a real side-by-side pilot (USS-TJR-MSN-0366 Stream 7): Kokoro-82M measured 2.63x realtime on an equivalent clip in the same sandbox, ~33x faster wall-clock. Kept running for cloning only, which Kokoro's fixed voice bank cannot do. The pip package (`chatterbox-tts` 0.1.7 on PyPI) does not yet include Nano support — this install is from GitHub master (unpinned commit), which does. Model download from Hugging Face was unreliable on this VM's network (repeated dropped connections mid-transfer over ~40 min, Xet backend hung entirely and had to be disabled via `HF_HUB_DISABLE_XET=1`) — worth knowing before assuming a future re-download will be quick.
- **Owner:** Chief Engineer.
...
- **Consumers:** none currently — `intelligence/scheduler.py`'s `_pregenerate_brief_audio()` (its one real caller) was moved to Kokoro TTS 2026-09-12, since it never set `voice_ref`. A future caller that needs cloning would still route here.
...
- **Capability Relationships:**
  ...
  - *Produces:* synthesized, voice-cloned speech (WAV), for future consumption by workbenches/LifeOS.
  - *Future Dependencies:* Notification (a candidate future consumer, for voice-based alerts); whichever workbench/LifeOS surface adopts voice cloning first.
...
- **Related Missions:** USS-TJR-MSN-0366 Stream 7 (re-scope to cloning-only, 2026-09-12); originally built ad hoc 2026-09-05 at Captain's request, not run as a formal mission.
- **Technical Debt:** unpinned git install (`chatterbox-tts` from GitHub master, no commit SHA pinned) — a future upstream change could silently alter behaviour or break `nano=True`; zero real adopters of the cloning path specifically; no concurrency/load testing; not yet linked into Discoverability/Monitoring/Governance per ADR-027 (no dashboard entry beyond this record, no health check wired to any monitoring surface).
- **Next Planned Evolution:** pick a first real cloning consumer (candidates raised: XO debrief audio in the Captain's own voice, Ready Room narration) and wire it end-to-end to prove adoption; pin the git install to a specific commit once stable.
- **Last Updated:** 2026-09-12.

### Voice Synthesis (Kokoro TTS)

- **Description:** local, self-hosted text-to-speech service using hexgrad's open-source Kokoro-82M model (Apache-2.0), run via the `kokoro-onnx` ONNX Runtime port (no torch/CUDA). Fixed voice bank, no cloning.
- **Purpose:** the "async pre-generated narration" niche Chatterbox was too slow for — briefs, debrief clips, any plain narration with no `voice_ref`. Added as a pilot under USS-TJR-MSN-0366 Stream 7 specifically to replace Chatterbox for this niche while leaving cloning to Chatterbox.
- **Engineering Confidence:** 45% — multiple real, timed, end-to-end generations via both direct inference and the live HTTP service (including a cache-hit round-trip) confirm the mechanism and its real speed; still a single-session pilot — no voice-quality review, no concurrent-request load test, no production adopter beyond the one real caller wired this session.
- **Current Maturity:** L1 — Implemented, one real adopter (`intelligence/scheduler.py`'s brief-audio pre-generation, moved from Chatterbox 2026-09-12).
- **Current Status:** built and pilot-verified this session (2026-09-12); FastAPI service, HTTP-verified with real generation calls (cold load + warm calls + cache hit) on this VM's CPU. Real measured speed: an 80-char sentence (comparable to Chatterbox's own ~90-char benchmark, which took ~35s / a 4-second clip took 48s) generated in 1.83s of raw inference (2.63x realtime), ~3.35s including full HTTP round-trip on a warm model — roughly 10-33x faster wall-clock than Chatterbox depending on what's measured. A longer 207-char paragraph generated in 14.11s (1.04x realtime), showing the realtime factor drops on longer text but stays well above Chatterbox's 0.08x. `pip install kokoro` (hexgrad's own torch-based package) was tried first per the pilot brief and rejected: it pulled a 5.9GB venv (full CUDA 13 stack as install-time deps even for CPU-only use) and, more critically, both it and the KittenTTS fallback fetch model weights from huggingface.co via `huggingface_hub` at runtime — and huggingface.co (all subdomains) is blocked outright by this deployment's egress proxy policy, confirmed via repeated 403 CONNECT refusals. `kokoro-onnx` wraps the same Apache-2.0 weights but ships them as GitHub Release assets instead, which this sandbox can reach; installed clean at 199MB with zero torch/CUDA/huggingface_hub. See `core/voice/tts_kokoro.py`'s header comment for the full trail.
- **Owner:** Chief Engineer.
- **Canonical Implementation:** `core/voice/tts_kokoro.py` (FastAPI wrapper, `/api/tts/generate`, `/api/tts/status`), dedicated venv at `core/voice/kokoro-venv/`, model weights at `core/voice/kokoro-models/` (downloaded from `github.com/thewh1teagle/kokoro-onnx` releases, not tracked in git — see `.gitignore`), port 8894 (`KOKORO_PORT`).
- **Consumers:** `intelligence/scheduler.py`'s `_pregenerate_brief_audio()` — moved here from Chatterbox 2026-09-12 (`KOKORO_SERVICE_URL`/`KOKORO_SERVICE_SECRET`, falling back to the old `TTS_SERVICE_URL`/`TTS_SERVICE_SECRET` names if unset).
- **Dependencies:** none in-platform (self-contained ONNX model + onnxruntime inference, CPU-only); GitHub Releases for the one-time model download (~340MB, cached locally after — not Hugging Face, see Current Status).
- **Capability Relationships:**
  - *Depends On:* none.
  - *Consumes:* nothing in-platform.
  - *Produces:* synthesized narration speech (WAV), consumed by the brief-audio pre-generation job; cache-key support matches Chatterbox's (`cache_key` -> sha256-named file under `core/voice/cache/`).
  - *Future Dependencies:* any future workbench/LifeOS narration surface (Hub/Captain's Chair "read aloud" already benefits indirectly via the pre-generated cache).
- **Related ADRs:** none yet — not formally reviewed against ADR-020/ADR-027; single real consumer so far.
- **Related Missions:** USS-TJR-MSN-0366 Stream 7 (this pilot, 2026-09-12).
- **Technical Debt:** unpinned model-file download (GitHub Release tag `model-files-v1.1`, no checksum verification in `tts_kokoro.py` beyond file-exists); `kokoro-onnx` itself is a third-party ONNX port of Kokoro-82M, not hexgrad's own package, so it lags upstream Kokoro releases; no voice-quality review against Chatterbox's cloned output (different capability, not directly comparable); no concurrency/load testing; not yet linked into Discoverability/Monitoring/Governance per ADR-027.
- **Next Planned Evolution:** if this pilot holds up under real production narration traffic, consider it the default narration path platform-wide (Hub, Captain's Chair, XO debrief text-only replies); pin the GitHub Release tag or add a checksum check; evaluate whether `kittentts` (15M params, smaller still) would suffice for shorter narration once its own weight-hosting situation is checked against this sandbox's egress policy.
- **Last Updated:** 2026-09-12.
...
- **Technical Debt:** graph sync is manual, not event-triggered; ghost `temporal_entities`/`facts`/`episodes` tables remain unresolved, with anon-executable `temporal_search_facts`/`temporal_search_episodes` RPCs flagged as an open security item across three prior missions — MSN-0210D still not scheduled. **RESOLVED 2026-09-08 (USS-TJR-MSN-0054):** the dormant second research-orchestration codebase (formerly `slack-bot/lib/research/`, later moved to `platform-runtime/lib/research/`) was deleted outright — confirmed zero live callers/importers repo-wide (only its own tests referenced it), and its one wired provider call (`_call_gemini_with_fallback`) was a hardcoded mock stub that never actually called a model. `core/coordination/research_orchestration.py` remains the sole, live, canonical research-orchestration implementation (confirmed called every 5 minutes by the systemd-supervised `intelligence-scheduler.service`). **MSN-0333, disclosed not fixed:** `lcars-portal/.../memory/[id]/route.ts`'s PATCH has no authorization check at all (any authenticated caller can reclassify/archive any document) — a real gap, out of this mission's retrieval-focused scope. `tools/supabase/retrieve_knowledge.py` still has zero live callers anywhere in the platform — sensitivity enforcement is now safe, but search itself still isn't reachable by the Captain through any real UI, only a manual CLI. Whether the Captain works through the 814-document review backlog is a usage question, not a code constraint.
- **Next Planned Evolution:** search readiness (wiring `retrieve_knowledge.py` into a real UI/chat surface) is now safely buildable, sensitivity enforcement no longer blocks it; a small officer-management UI once a second real officer is onboarded; event-triggered `knowledge_navigation` sync.
...
- **Engineering Confidence:** 70% (revised down from 78%, 2026-09-12, USS-TJR-MSN-0365) — proven end-to-end with a real test write reaching all 6 tables in the chain (`commander_decisions`→`decision_outcomes`→`decision_records`→`quality_scores`→`feedback_signals`→`provider_quality_history`) at the time that test was written, but `quality_scores` itself was dropped outright in migration `0183_drop_retired_dead_tables.sql` (2026-09-01) and `score_outcome()`'s `INSERT INTO quality_scores` has been silently failing (caught by its own broad `except`, logged as an error, not surfaced anywhere else) since — found during PR #114's investigation of a related, narrower claim ("no LLM output is scored in production"). Adoption is still narrow (2 domains) and the schema relationships took real debugging to get right originally, but the chain this record describes as "proven end-to-end" no longer reaches its last table.
...
- **New, MSN-0365 (2026-09-12):** `quality_scores` (the chain's own write target) was dropped in migration `0183_drop_retired_dead_tables.sql` on 2026-09-01. `score_outcome()` (`platform-runtime/lib/quality_scoring_service.py`, fires from all 3 learning loops) still issues `INSERT INTO quality_scores` on every call — now failing 100% of the time, silently, via a broad `except`. This is distinct from and more severe than the "0 rows since test cleanup" status this record previously carried: there is no longer a table to have rows in. Separately, `score_output()` (the deepeval/`HallucinationMetric` path, a different scoring mechanism entirely — see Observability capability) had zero callers and no judge model configured; both are now fixed (PR #114) but write nowhere durable (shadow-mode log line only), so neither of Confidence's two scoring paths currently persists anything.
- **Next Planned Evolution:** **Fix Now (raised from Fix Later, 2026-09-12):** either restore a real write target for `score_outcome()` (a new migration, or repoint it at whatever replaced `quality_scores` if anything did) or deliberately retire the dead `INSERT` so the failure stops being silent — either is better than the current state of a "proven end-to-end" capability whose last table doesn't exist. Once resolved: extend wiring to remaining decision domains (Health Intelligence, Operational Resilience Intelligence) as those domains' ownership questions resolve; confirm a real (non-test) event has flowed through the fixed chain; fix the provider/model attribution gap; decide the fate of the dead `register_capability()` mechanism; apply the Confidence Naming Decision to any future domain emitter (never map a domain-local "confidence"-named field onto `core_events.confidence` without checking it actually means epistemic trust first).
- **Last Updated:** 2026-07-05.
...
- **Technical Debt:** **corrected 2026-09-08 (USS-TJR-MSN-0054 discovery):** these were never really 3 competing variants of the same thing — the earlier framing grouped them by shared vocabulary (task/queue), not function. `slack-bot/lib/research/task_executor.py` was a dormant, never-finished duplicate of the live `research_orchestration.py` and has been deleted outright (see Knowledge capability's Technical Debt). `core/coordination/number_one.py`'s `WorkQueueItem` is mission-triage (project management over the Mission Registry), not research-task execution — it doesn't compete with `research_orchestration.py` at all. It's now live: wired into the Context Assembly Service's Number One brief endpoint (see Context Assembly / Number One Brief below) rather than left dormant. Neither has anything to migrate onto the Task Engine substrate today — one is deleted, the other has zero live callers to migrate from. `event_bus.py`/`unified_memory.py`/`relationship_model.py` likely carry the same raw_client-dependency bug just fixed here — not yet audited/fixed for the other 3 modules.
...
- **Current Status:** systemd timers correctly handle infra-level *scheduling* (firing on time); the automation one such timer (`hq-evolution.timer`, 03:00 daily) *executes* had a real bug, now fixed (LL-149, USS-TJR-MSN-0365, 2026-09-12) — `auto_remediation.py`'s `git_commit()` had no branch check and no push, so its cycle-artifact commits landed silently on whatever branch was checked out on the VM at 03:00 and never reached `origin` (diagnosed as LL-146, fixed here: refuses to commit unless `HEAD` is a dedicated `self-improvement` branch, pushes on success, fails closed and logs loudly otherwise). This was an automation-correctness bug, not a scheduling-mechanism bug — the timer itself always fired correctly. 5 separate in-process `apscheduler` instances still exist with no coordination, untouched by this fix.
- **Owner:** TBD.
...
- **Canonical Implementation:** `intelligence/` (ingestion adapters, classification, ranking, `scheduler.py`, `captains_brief.py`), plus `core/platform/interrupt_dispatcher.py` and `core/platform/attention_engine.py`'s `continuous_attention_evaluation` job (MSN-0339 WP2/WP3). **Brief approval gate, not previously listed here:** `intelligence/brief/brief_generator.py` (`_compute_risk`/`_to_brief_event`), `intelligence/audit/brief_qa_agent.py` (nightly QA pre-screen), `intelligence/audit/brief_coherence.py` (the coherence checks QA runs) — govern whether a generated `intelligence_briefs` row ever reaches `PUBLISHED` (migration `0084`'s IN_REVIEW → QA_PASSED → PUBLISHED ladder).
...
- **Technical Debt:** (1) Telstra itself has no public unauthenticated feed — structurally uncollectible, disclosed not fixed. (2) The shared 129-source registry is contaminated with 26 sources tagged "wellness" — unrelated ingestion content feeding a banking-tuned classifier, likely belonging to Wellness Coaching or Health Intelligence instead — not re-verified since MSN-0302. (3) Two independent, uncoordinated schedulers still exist (`intelligence/scheduler.py` vs `platform-runtime/proactive_scheduler.py`) — deferred across MSN-0339/340, unresolved. (4) `executive_relevance` is `NULL` on every recent `intelligence_events` row checked — a populated-schema-but-empty-data gap. (5) **Found 2026-08-19, undisclosed until now:** the brief approval gate (see Canonical Implementation) had 0 briefs pass nightly QA since 2026-06-20 — `brief_coherence.py`'s `snapshot_reflects_events` check required an exact 20-char substring of a top-event title inside the LLM-paraphrased narrative, which essentially never matched. This 85% confidence figure was set (MSN-0343, 2026-07-08) without visibility into this — the ingestion→classification→ranking chain it describes was healthy, but the brief's own publish path was silently jammed the whole time. Fixed (fuzzy keyword-overlap check, plus a risk-floor bug and a QA-sampler truncation bug found in the same pass); not yet re-observed against a live nightly run to confirm briefs now actually reach PUBLISHED.
- **Next Planned Evolution:** await MSN-0341's operational-acceptance decision; decide ownership/placement of the 26 wellness-tagged sources; the dual-scheduler consolidation (item 3) is a strong next-mission candidate per MSN-0342's roadmap; confirm item (5)'s fix produces a real PUBLISHED brief on the next nightly QA cycle.
- **Last Updated:** 2026-08-19 (brief approval gate found broken since 2026-06-20 and fixed, see Technical Debt; prior update 2026-07-08, MSN-0343).
...
- **Engineering Confidence:** 30% — the design is coherent and traces directly to a real observed incident, but nothing of the *policy-decision* mechanism itself is built. Data Classification & Model Routing (the capability this depends on) moved to L2 on 2026-09-12 (USS-TJR-MSN-0366 Stream 5) — but for a real cloud-egress redaction/blocking control, not the classification-declaration/routing-policy mechanism this entry actually needs; that dependency is still functionally unmet.
...
- **Description:** a 6-tier data-sensitivity classification (Public/Internal/Confidential/Security Sensitive/Captain Only/Secrets) plus a policy layer generalising Model Router's routing beyond pure performance to include classification, provider trust, and engineering confidence. **USS-TJR-MSN-0366 Stream 5 (2026-09-12) built a real, narrower down-payment on this idea**: automatic (not declared) sensitive-identifier detection at the Model Router's actual cloud-egress boundary, plus a real block-on-injection input rail — see below.
...
- **Engineering Confidence:** 50% (up from 35%) — the automatic-detection half of this capability (previously an acknowledged, deliberately-deferred gap: "classification is manual/declared only") now has a real implementation for two identifier types plus Presidio's built-in PII types, and a real LLM01 input rail exists where none did before. Still well short of the full 6-tier design: no `Public`/`Internal`/`Confidential`/`Captain Only`/`Secrets` tier model, no per-artefact declared classification, no provider-trust or engineering-confidence routing inputs, and coverage is real but partial (see Technical Debt).
- **Current Maturity:** L2 — Implemented, narrow scope, not yet deployed to production.
- **Current Status:** `core/security/llm_guardrails.py` (+ `core/security/_llmsec_worker.py`, `core/security/guardrails/`) wraps `core/model-router/app.py`'s `_gemini_generate` dispatch and 2 of 3 real `mistralai` cloud client modules (`core/engineering/providers/mistral_batch.py`, `mistral_batch_api.py`) with: (1) Presidio PII/PHI redaction, including two TJR-specific recognizers (`TJR_MISSION_CODENAME`, `TJR_OFFICER_CLEARANCE`) Presidio's built-ins have no way to know about; (2) a NeMo Guardrails input rail (deterministic keyword/regex pre-filter + a semantic self-check using this platform's own local `gemma3:4b` via Ollama — no new model hosted); (3) a matching output rail (credential-leak regex + semantic unsafe-content check). Fails closed if its isolated venv (`platform-runtime/.venv-llmsec`) isn't provisioned. Full details and OWASP Top 10 mapping: `docs/decisions/ADR-llm-application-security-baseline.md`. Real end-to-end evidence (not yet against a live Ollama — see that ADR's Gaps section): `core/security/test_llm_guardrails.py`, `tests/test_model_router_guardrails.py`.
- **Owner:** Chief Engineer.
- **Canonical Implementation:** the redaction/rail layer: `core/security/llm_guardrails.py`. The original 6-tier classification design remains undelivered: `reports/USS-TJR-MSN-0210M-Secure-Execution-Policy-Architecture.md` Workstreams B/C.
- **Consumers:** `core/model-router/app.py` (Gemini branch), `core/engineering/providers/mistral_batch.py`, `core/engineering/providers/mistral_batch_api.py`. Secure Execution Policy and a full Model Routing Policy still have no consumer relationship to this capability — Stream 5 built a data-egress control, not the routing-decision layer this capability was originally scoped to be.
- **Dependencies:** Model Router (routing sits above it, doesn't replace it), Execution Runtime Registry.
- **Capability Relationships:**
  - *Depends On:* Execution Runtime Registry (`allowed_classifications` field) — still true, still unbuilt.
  - *Consumes:* task `input_context`'s declared classification (original design — not what Stream 5 built, which detects automatically instead of consuming a declaration).
  - *Produces:* routing/refusal decisions (original design) + real redact/block decisions on cloud-egress prompts (Stream 5, real).
  - *Future Dependencies:* Secure Execution Policy (consumes this capability's output) — still unbuilt, still not wired to Stream 5's work.
- **Related ADRs:** `docs/decisions/ADR-llm-application-security-baseline.md` (2026-09-12, USS-TJR-MSN-0366 Stream 5).
- **Related Missions:** MSN-0210I (the finding that motivated the original design — originally logged as a one-line Planned Pipeline entry, now a full record), MSN-0210M (designed, Workstreams B/C), USS-TJR-MSN-0366 Stream 5 (2026-09-12, this update).
- **Technical Debt:** the full 6-tier classification model, provider-trust inputs, and cost/availability routing inputs remain entirely unbuilt — Stream 5 solved a real, narrower problem (stop unredacted/unchecked prompts leaving the VM via 2 of 3 known mistralai clients + the Gemini branch), not the general capability this entry names. Known, disclosed gaps in Stream 5's own work: `glm-*:cloud` Ollama-routed models are NOT covered (same code path as fully-local calls — see the ADR); `platform-runtime/lib/mistral_agent_client.py` and three other inline `from mistralai import Mistral` call sites are NOT wrapped; the semantic (model-backed) half of both rails is verified only against NeMo Guardrails' `FakeLLMModel`, not a live Ollama call (none reachable from the sandbox this was built in).
- **Next Planned Evolution:** provision `platform-runtime/.venv-llmsec` on the real production host and verify the semantic rail against a live Ollama call (the one thing this stream could not verify from its sandbox); then extend coverage to `mistral_agent_client.py`; then, separately, revisit whether the original 6-tier classification design is still the right shape given what Stream 5 learned building a real (if narrower) version of it.
- **Last Updated:** 2026-09-12 (USS-TJR-MSN-0366 Stream 5).
...
- **Next Planned Evolution:** per MSN-0342/0343's Captain Brief Convergence finding — this module, `captain_brief_evolution.py`, and `intelligence/captains_brief.py` should be formally documented as one architecture (canonical base / reasoning layer / legitimate separate producer) rather than three independently-tracked capabilities; re-validate once Engineering/Research/Content Intelligence gain real emit volume. **Still not done as of 2026-08-19** — that session's fixes touched `captain_brief_evolution.py` (Captain Intelligence Core record) and `intelligence/brief/brief_generator.py`/`intelligence/audit/*` (Operational Resilience Intelligence record) without resolving this convergence-documentation debt; the three-pipeline overlap this item describes is still real.
- **Last Updated:** 2026-07-08 (MSN-0343).
...
- **Technical Debt:** `insight_outcomes` has 2 real rows (both from engineering validation, not yet organic Captain usage) — the Operational Observation Period's 20-row/10-day threshold still governs when any tuning/confidence-scoring work may start; that has not changed. `core_events` still concentrated in one domain in practice — the newer emitters (missions/delivery/strategy/comms/knowledge/research) have zero real rows so far. Automation domain has no clean choke point — disclosed, not implemented. `model-router.service` is real infra shared with other live bots — restart deliberately, not casually. Context Assembly Foundation's captain-brief-specific consumers remain grandfathered per Option C. LCARS generate route's 290s client timeout assumes no shorter reverse-proxy/serverless timeout sits in front of it — not re-verified against this platform's actual deployment target this pass.
  - **2026-08-19 (2 disclosed changes, not a tuning pass):** (1) `insight_outcomes.record_outcome()` was built in Phase 4 to close the pending→reviewed loop but had zero callers anywhere — every insight sat `outcome='pending'` forever, so the Operational Observation Period could never accumulate real Captain-judged signal even in principle. Added `POST /api/captain-intelligence/insights/[id]/outcome` (its first real caller) and Useful/Not Useful/Incorrect buttons on `CaptainIntelligencePanel`. (2) `generate_insights()` was synthesizing an insight from every 'aggregation'-kind Relationship on first occurrence — its evidence is only ever a same-batch count with no content field to synthesize from, so this produced generic "review and prioritize the N signals" filler every time (confirmed live from a real pending card). Excluded 'aggregation' kind from the candidate list entirely; removed the now-purposeless `recent_aggregation_keys` dedup parameter and its `fetch_recent_aggregation_keys()` fetch (`insight_outcomes.py`, `captain_brief_evolution.py`). Both changes close structural gaps in how real signal reaches/leaves this pipeline — neither adjusts a threshold or confidence score, so the evidence-threshold gate itself is untouched. No mission number minted for this pass (ad hoc Captain-directed session); flagging that gap here rather than inventing one.
- **Next Planned Evolution:** wait for real Captain's Chair usage to accumulate real `insight_outcomes` history — now more likely to actually happen given (1) above; re-run the regression harness after any future prompt/threshold change (none planned until the evidence threshold is met); revisit Automation's domain coverage only if a specific real need arises.
- **Last Updated:** 2026-08-19 (disclosed dead-loop/noise fixes, see Technical Debt; prior update 2026-07-07).
