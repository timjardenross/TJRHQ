...
**Last synced:** HQ V1 Integration QA (this mission) — added the 2 missing rows (`/hub`, `/physical-readiness`) and corrected 2 stale titles (`Human Systems` and `Advisory`, previously listed here as `Human Systems Workbench`/`Advisory Workbench`) to match `workbenches.ts`'s current `LIVE_WORKBENCHES` array exactly. A follow-on workbench integration audit later in the same mission corrected `/physical-readiness`'s description, which still claimed a "builds a safe session" capability retired weeks earlier (Captain directive, 2026-08-10/11). Also carries the 2026-09-04 sync that added `/privacy` and `/terms` (Google Calendar OAuth verification pages, LifeOS wall tablet §2.7).
