...
/** Three-way domain switch. Mirrors the Intelligence Workbench toggle; keyboard
 *  navigable with role=tablist semantics for WCAG 2.1 AA (design proposition §9). */
export function DomainToggle({ domain, onChange }: { domain: Domain; onChange: (d: Domain) => void }) {
  return (
    <div role="tablist" aria-label="Human Systems domain" className="flex gap-1 rounded-md border border-wb-line bg-wb-surface px-1.5 py-1">
      {DOMAINS.map((d) => {
