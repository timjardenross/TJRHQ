...
type TopTab = 'consult' | 'board' | 'brief' | 'picture' | 'intelligence';
...
// ── Operational card (for Picture mode) ───────────────────────────────────────
...
const COUNCIL: CouncilAdvisor[] = [
  ...
  { id: 'recovery_officer', label: 'Recovery Officer', subtitle: 'Recovery & Directive 055', accent: 'text-medical', group: 'Wellness' },
];

const LS_CONSULT_KEY = 'lcars-council-consult-history';

// ── Consult mode (direct advisor chat) ────────────────────────────────────────
...
function ConsultMode() {
  ...
  }, [threads, activeAdvisor, streamBuffer]);
  ...
  const send = useCallback(async (text: string) => {
    ...
    const currentThread = threads[activeAdvisor.id] ?? [];
    const history = [...currentThread, userMsg];
    ...
    try {
      ...
      while (true) {
        ...
        for (const line of lines) {
          ...
          try {
            ...
            if (parsed.token) { acc += parsed.token; setStreamBuffer(acc); }
          } catch { /* skip */ }
        }
      }
      ...
      setThreads((prev) => ({
        ...
        [activeAdvisor.id]: [
          ...
          { id: (Date.now() + 1).toString(), role: 'assistant', content: acc || '(no response)' },
        ],
      }));
      setStreamBuffer('');
    } catch (err) {
      ...
      if (e.message.includes('fetch') || e.message.includes('ECONNREFUSED')) {
        ...
        setThreads((prev) => ({
          ...
          [activeAdvisor.id]: [
            ...(prev[activeAdvisor.id] ?? []),
            { id: (Date.now() + 1).toString(), role: 'assistant', content: 'Error contacting advisor.', error: true },
          ],
        }));
      }
    } finally {
  ...
  const clearThread = () => {
    ...
    try { localStorage.setItem(LS_CONSULT_KEY, JSON.stringify({})); } catch { /* ignore */ }
  };

  const isOffline = offlineAdvisors.has(activeAdvisor.id);

  // Group advisors by group label
  ...
  return (
    <div style={{ display: 'flex', gap: '1rem', height: '65vh' }}>
          ...
          <div key={group}>
            ...
            <div className="flex flex-col gap-1">
              {advisors.map((a) => (
                <button
                  key={a.id}
                  onClick={() => { setActiveAdvisor(a); setStreamBuffer(''); }}
                  className={[
                    'rounded-lcars border px-2.5 py-2 text-left transition-colors',
      ...
      {/* Thread panel */}
      <LCARSPanel
        title={`${activeAdvisor.label} — ${activeAdvisor.subtitle}`}
        accent="command"
          ...
          <div className="flex flex-col gap-3 h-full">
            <div className="flex-1 overflow-y-auto space-y-3 pr-1">
              {messages.length === 0 && !loading && (
                <p className="text-lcars-muted text-sm text-center py-8">
                  {activeAdvisor.label} standing by.
                </p>
              )}
              {messages.map((m) => {
              ...
              {loading && streamBuffer && (
                <div style={{ display: 'flex', justifyContent: 'flex-start' }}>
                  <div className="max-w-[90%] rounded-lcars border border-edge bg-panel/60 px-3.5 py-2.5 text-sm leading-relaxed text-lcars-text/90">
                    <p className={`mb-1 text-[10px] uppercase tracking-[0.2em] ${activeAdvisor.accent}`}>{activeAdvisor.label}</p>
                    <div className="prose prose-sm prose-invert max-w-none prose-p:my-1 prose-headings:text-lcars-text prose-headings:font-lcars prose-strong:text-lcars-text prose-li:my-0.5">
            ...
            <div style={{ display: 'flex', alignItems: 'flex-end', gap: '0.5rem' }}>
              <textarea
                ...
                onKeyDown={onKeyDown}
                rows={2}
                placeholder={`Message ${activeAdvisor.label}…`}
                disabled={loading}
...
// ── Board mode (multi-advisor advisory council) ────────────────────────────────

interface BoardResult {
  summary: AdvisoryResult | null;
  error: string | null;
}
...
function BoardMode() {
  ...
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<BoardResult>({ summary: null, error: null });
  ...
  const submit = async () => {
    ...
    try {
      const res = await fetch('/api/advisory', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'advice', question }),
      });
      const data = (await res.json()) as { result?: AdvisoryResult; error?: string };
      if (data.error) { setResult({ summary: null, error: data.error }); return; }
      setResult({ summary: data.result ?? (data as unknown as AdvisoryResult), error: null });
    } catch (err) {
      setResult({ summary: null, error: (err as Error).message ?? 'Unknown error' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const perspectives = result.summary?.officer_perspectives ?? [];
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-3 mb-2">
            ...
            <button
              onClick={() => setIsScenario(false)}
              className={`rounded-lcars border px-3 py-1 text-[10px] uppercase tracking-widest transition-colors ${
                !isScenario ? 'border-command bg-command/10 text-command' : 'border-edge text-lcars-muted hover:border-edge/60'
              }`}
            >
              Advisory
            </button>
            <button
              onClick={() => setIsScenario(true)}
              className={`rounded-lcars border px-3 py-1 text-[10px] uppercase tracking-widest transition-colors ${
                isScenario ? 'border-science bg-science/10 text-science' : 'border-edge text-lcars-muted hover:border-edge/60'
              }`}
            >
              Scenario
            </button>
            {isScenario && (
              <span className="text-[10px] text-science/70 italic">What would happen if…</span>
            )}
          </div>
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: '0.5rem' }}>
            <textarea
              ...
              onKeyDown={onKeyDown}
              rows={2}
              placeholder={isScenario ? 'What would happen if…' : 'Bring a question to the Advisory Board…'}
              disabled={loading}
              className="flex-1 resize-none rounded-lcars border border-edge bg-space px-3 py-2 text-sm text-lcars-text placeholder:text-lcars-muted focus:border-science focus:outline-none disabled:opacity-50"
            />
        ...
        {result.error && (
          <div className="rounded-lcars border border-operations/40 bg-operations/10 px-4 py-3 text-sm text-operations">
            ...
            <p className="text-operations/80 text-xs">{result.error}</p>
          </div>
        )}
        ...
        {result.summary && !loading && (
          <div className="space-y-4">
            {/* Overall summary */}
            <AdvisoryBlock data={result.summary} />

            {/* Officer perspectives grid */}
                    ...
                    return (
                      <div key={i} className="rounded-lcars border border-edge bg-panel/50 px-3 py-2.5 space-y-1.5">
                        ...
                        {op.confidence !== undefined && (
                          <p className="text-[9px] text-lcars-muted">Confidence: {op.confidence}%</p>
                        )}
                      </div>
                    );
                  })}
    ...
    >
      ...
      {error && (
        <div className="rounded-lcars border border-operations/40 bg-operations/10 px-4 py-3 text-sm text-operations">
          ...
          <p className="mt-2 text-lcars-muted text-xs">Switch to Board tab for direct officer consultation.</p>
        </div>
      )}
...
function PictureMode() {
  ...
  const fetchPicture = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/advisory', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'captains-picture' }),
      });
  ...
  return (
    <LCARSPanel title="Operating Picture" accent="command"
      actions={
        <button onClick={fetchPicture} disabled={loading}
          className="rounded-lcars border border-edge px-3 py-1 text-[10px] uppercase tracking-widest text-lcars-muted hover:text-command transition-colors disabled:opacity-40">
          Refresh
        </button>
      }
    >
      {loading && (
        <div className="flex items-center gap-3 py-8">
          {[0, 1, 2].map((i) => (
            <span key={i} className="h-2 w-2 animate-pulse rounded-full bg-command"
              style={{ animationDelay: `${i * 150}ms` }} />
          ))}
          <span className="text-lcars-muted text-sm">Assembling operating picture…</span>
        </div>
      )}
      {error && (
        <div className="rounded-lcars border border-operations/40 bg-operations/10 px-4 py-3 text-sm text-operations">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-lcars-muted tracking-wider mt-0.5">
            Captain&apos;s Advisory Council — Starship Endeavour
          </p>
        </div>
      </div>
