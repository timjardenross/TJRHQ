...
export default function WorkoutHistoryPage() {
  ...
  if (loading) return <p className="p-6 text-center text-sm text-lcars-muted">Loading history…</p>;
  ...
  return (
    <div className="flex flex-col gap-4">
      <LCARSPanel title="Workout History" accent="medical" eyebrow={`${sessions.length} recent session${sessions.length === 1 ? '' : 's'}`}>
        {sessions.length === 0 && <p className="text-sm text-lcars-muted">No sessions logged yet.</p>}
      </LCARSPanel>
      ...
      {(cardioInsight || durationInsight) && (
        <LCARSPanel title="Patterns" accent="medical" eyebrow="Observed, not prescribed">
          <ul className="flex flex-col gap-2 text-xs text-lcars-muted">
      ...
      <LCARSPanel title="Most Used Exercises" accent="medical">
        {mostUsed.length === 0 && <p className="text-xs text-lcars-muted">No completed exercises logged yet.</p>}
        <ul className="flex flex-col gap-1.5">
          {mostUsed.map((e) => (
            <li key={e.name} className="flex items-center justify-between text-sm">
              <span className="text-lcars-text">{e.name}</span>
              <span className="text-lcars-muted">{e.count}x</span>
            </li>
          ))}
        </ul>
      </LCARSPanel>
      ...
      {mostSkipped.length > 0 && (
        <LCARSPanel title="Most Skipped" accent="medical">
          <ul className="flex flex-col gap-1.5">
            {mostSkipped.map((e) => (
              <li key={e.name} className="flex items-center justify-between text-sm">
                <span className="text-lcars-text">{e.name}</span>
      ...
      {painFlagged.length > 0 && (
        <LCARSPanel title="Exercises Trending Higher Pain" accent="medical" eyebrow="Avg pain during ≥ 5/10">
          <ul className="flex flex-col gap-1.5">
            {painFlagged.map((e) => (
              <li key={e.name} className="flex items-center justify-between text-sm">
                <span className="text-lcars-text">{e.name}</span>
                <StatusBadge label={`${e.avgPain.toFixed(1)}/10`} tone="operations" />
              </li>
            ))}
          </ul>
        </LCARSPanel>
      ...
      <LCARSPanel title="Recent Sessions" accent="medical">
        <ul className="flex flex-col gap-2">
          {sessions.map((s) => (
            <li key={s.id} className="flex items-center justify-between rounded-lcars border border-edge bg-space/40 px-3 py-2 text-sm">
              <div>
                <p className="font-semibold text-lcars-text">{SESSION_TYPE_LABELS[s.session_type] ?? s.session_type}</p>
                <p className="text-[11px] text-lcars-muted">
                  {fmtDate(s.started_at)}{s.duration_minutes ? ` · ${s.duration_minutes} min` : ''}
                </p>
              </div>
              <StatusBadge label={s.status.replace('_', ' ')} status={s.status} />
            </li>
          ))}
        </ul>
      </LCARSPanel>
