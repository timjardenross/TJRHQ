...
def compute_domain_picture(top_events: list[dict]) -> Optional[dict]:
    """
    top_events: list shaped like intelligence_briefs.top_events (title,
    event_type, risk_rating, summary, ...).
    Returns {bucket_key: {"label": str, "count": int, "worst_risk": str,
    "events": [{"title", "risk_rating"}]}}, or None if there are no events
    to group (nothing fabricated for an empty brief).
    """
    if not top_events:
        return None
    ...
    for event in top_events:
        bucket = _bucket_for(event.get("event_type", ""))
        entry = picture.setdefault(bucket, {
            "label": _DOMAIN_LABELS[bucket], "count": 0, "worst_risk": "GREEN", "events": [],
        })
        entry["count"] += 1
        entry["events"].append({"title": event.get("title"), "risk_rating": event.get("risk_rating")})
        risk = (event.get("risk_rating") or "GREEN").upper()
